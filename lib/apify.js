...
async function fetchTikTokPosts(usernames, resultsPerPage = 30) {
  // Input shape matches clockworks/tiktok-profile-scraper's documented schema.
  // Double check this against the actor's "Input" tab if it stops returning data —
  // input schemas are the most common thing that silently changes.
  const input = {
    profiles: usernames,
    resultsPerPage,
