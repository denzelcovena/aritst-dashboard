...
/**
 * Maps a apify/instagram-followers-count-scraper dataset item into an
 * instagram_stats row. Field name is a best guess (followersCount) —
 * verify against a real test run in the Apify console before trusting it,
 * same as we did for TikTok and Spotify.
 */
async function upsertInstagramStats(rawItems) {
  const item = rawItems[0];
  if (!item) {
    return { count: 0 };
  }
  ...
  if (error) throw new Error(`Supabase upsert (instagram_stats) failed: ${error.message}`);
  return { count: data.length };
}

module.exports = { upsertTikTokPosts, upsertSpotifyStats, upsertInstagramStats };
